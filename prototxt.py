...
def save_prototxt(net_info, protofile, region=True, fake = True):
    fp = open(protofile, 'w')
    # whether add double quote
    def format_value(value):
        #str = u'%s' % value
    ...
    props = net_info['props']
    layers = net_info['layers']
    layer_dims= net_info['layer_dims']
    ...
    for layer in layers:
        ...
        if (layer['type'] != 'DetectionOutput') and layer['type'] !='Reorg':
            print_block(layer, 'layer', 0)
        elif layer['type'] =='Reorg' :
            ##feak reorg
            if fake==True:
                layer_fake = layer.copy()
                layer_fake['type'] = 'Reshape'
                reshape_param = OrderedDict()
                print(layer_dims[layer_num])

                shape=[1,int(layer_dims[layer_num]['channels']),int(layer_dims[layer_num]['height']),int(layer_dims[layer_num]['width'])]
                reshape_param['shape {']='##_##'
                reshape_param['dim'] =shape
                reshape_param['}']='##_##'
                layer_fake['reshape_param'] = reshape_param


                del layer_fake['reorg_param']
                print_block(layer_fake,'layer',0)
            else:
                print_block(layer,'layer',0)
        elif layer['type'] =='DetectionOutput' and region == True:
            print_block(layer, 'layer', 0)

        if  layer['type'] =='Concat' or \
            layer['type'] == 'Pooling' or\
            layer['type'] == 'Reorg' or\
